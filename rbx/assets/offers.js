...
function CPBContentLocker() {
  ...
  this.settings =
    typeof window["CPABUILDSETTINGS"] === "object"
...
CPBContentLocker.prototype = {
  ...
  onSettingsAndHTMLLoaded() {
    ...
    //escape key
    if (
      parseInt(this.getValue("escape_key_close")) === 1 ||
      this.settings.testing === 1 ||
      this.settings.escapeOverwrite === true
    ) {
  ...
  onVarsChange: function () {
    ...
    if (typeof this.defaultSettings["content_locker_title_text"] != "undefined")
      document.getElementById("CPABUILDMODALTITLE").innerHTML =
        thisPass.getValue("content_locker_title_text");
    if (
      typeof this.defaultSettings["content_locker_footer_text"] != "undefined"
    )
      document.getElementById("CPABUILDMODALFOOTERTEXT").innerHTML =
        thisPass.getValue("content_locker_footer_text");
    if (
      typeof this.defaultSettings["overlay_color"] != "undefined" &&
      typeof this.defaultSettings["overlay_opacity"] != "undefined"
    ) {
      ...
      document.getElementById("CPABUILDSettingsCSS").innerHTML +=
        "#CPABUILD_MODAL_CONTAINER #CPABUILD_MODAL {background-color: " +
        rgba +
        " }";
    }
    ...
    if (typeof this.userSettings["number_offers_required"] != "undefined") {
      this.requiredLeads = parseInt(
        this.userSettings["number_offers_required"]
      );
    }
  ...
  getValue: function (key) {
    ...
    if (
      typeof this.userSettings[key] !== "undefined" &&
  ...
  offersComplete: function (data) {
    ...
    if (typeof this.settings.redirect !== "undefined") {
      ...
      window.location = url;
      this.changeHTML(
        "Offer complete! <a href='" + url + "'>Click here</a> to continue"
      );
      return false;
    }
  ...
  getIframeHTML: function () {
    return (
      '<iframe src="' +
      this.urls.iframeOnly +
      '" height="100%" marginwidth="0" marginheight="0" align="middle" frameborder="0" width="100%" ></iframe>'
    );
  },
  ...
  setURLS: function () {
    ...
    //   q +
    //   "&" +
    //   this.getParamString(["it", "w", "h", "key", "m", "r"]);
    ...
    //   q +
    //   "&" +
    //   this.getParamString(["it", "w", "h", "key", "m", "callback"]);
    ...
    //   p +
    //   "external/v2/" +
    //   (this.settings.preview === true ? "preview" : "html") +
    //   "." +
    //   this.settings.it +
    //   "." +
    //   this.settings.key +
    ...
    // this.urls.check =
    //   p + "external/check.php?" + this.getParamString(["it"]) + "&time=";
    // this.urls.impression =
    //   p + "external/impression.php?" + this.getParamString(["it"]) + "&time=";
